...
def update():
    ...
    t_cases,t_cured,t_deaths=0,0,0
    
    for key,value in response.items():
        if "unassigned" in key.lower():
            continue
        state = key
        value = value['districtData'].values()
        cases,cured,death=0,0,0
        ...
        t_cured+=cured
        t_deaths+=death
        print([state, cases,cured,death])
        # print("\n\n")
        try:
            update_state(conn,state,cases,cured,death)
        except:
            continue
    
    
    # update_total(conn,int(data[0])-int(olddata["cases"]),int(data[1])-int(olddata["cured"]),int(data[2])-int(olddata["death"]))
    gen_new_report([t_cases,t_cured,t_deaths])
    # gen_new_report()
